...
(
    ...
    codescuela text COLLATE pg_catalog."default" NOT NULL,
    direccionest text COLLATE pg_catalog."default" NOT NULL,
    ...
    CONSTRAINT fk_cod_escuela FOREIGN KEY (codescuela)
        REFERENCES public.escuela (codescuela) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    CONSTRAINT restriccionstatusest CHECK (statusest = ANY (ARRAY['A'::text, 'R'::text, 'N'::text, 'E'::text]))
)
...
ALTER TABLE IF EXISTS public.estudiantes
    OWNER to postgres;
-- Index: idx_cod_escuela

-- DROP INDEX IF EXISTS public.idx_cod_escuela;

CREATE INDEX IF NOT EXISTS idx_cod_escuela
    ON public.estudiantes USING btree
    (codescuela COLLATE pg_catalog."default" ASC NULLS LAST)
    TABLESPACE pg_default;
